...
public class ANOVA2_JustBarsView extends Region { 
    // POJOs
    boolean dragging;
    ...
    ANOVA2_JustBarsView(ANOVA2_Model twoWayANOVAModel, String choiceOfPlot, String meansOrBars) {
        ...
        xAxis = new CategoryAxis(this.twoWayANOVAModel.getFactorALevels());
        xAxis.setSide(Side.BOTTOM); 
        xAxis.setAutoRanging(true);
        xAxis.setLabel(twoWayANOVAModel.getFactorALabel());
        ...
        yAxis.setLabel(twoWayANOVAModel.getResponseLabel());
        ...
        switch (choiceOfPlot) {
            ...
            case "StandardDeviation":   // Bar +/- sd
                scene.heightProperty().addListener(ov-> {doDotsOrRectsWithErrorBars();});
                scene.widthProperty().addListener(ov-> {doDotsOrRectsWithErrorBars();});  
                strErrBarDescr = "mean +/- standard deviation";
                yAxis.forceLowScaleEndToBe(0.0);
                doDotsOrRectsWithErrorBars(); 
                break;
                
            case "StandardError":       // Bar +/- se
                scene.heightProperty().addListener(ov-> {doDotsOrRectsWithErrorBars();});
                scene.widthProperty().addListener(ov-> {doDotsOrRectsWithErrorBars();});  
                strErrBarDescr = "mean +/- standard error";
                yAxis.forceLowScaleEndToBe(0.0);
                doDotsOrRectsWithErrorBars(); 
                break;
            ...
            case "BarAndMean": 
                scene.heightProperty().addListener(ov-> {doBarAndMean();});
                scene.widthProperty().addListener(ov-> {doBarAndMean();});
                strErrBarDescr = "Values are means";
                yAxis.forceLowScaleEndToBe(0.0);
                doBarAndMean(); 
                break;
    ...
    private void doDotsOrRectsWithErrorBars() {  
            ...
            for (int theWithinBatch = 1; theWithinBatch <= nFactorB_Levels; theWithinBatch++) {
                ...
                */
                ...
                switch (choiceOfPlot) {
                    ...
                    case "StandardError":
                        errorBarLength = tempUCDO.getTheStandDev() / tempUCDO.getLegalN();
                        break;
                ...
                double topOfErrorfBar = yAxis.getDisplayPosition(tempUCDO.getTheMean() + errorBarLength);
                double bottomOfErrorBar = yAxis.getDisplayPosition(tempUCDO.getTheMean() - errorBarLength);
                ...
                if (meansOrBars.equals("Means")) {  // Error bars drawn last
                    ...
                    double theMean = yAxis.getDisplayPosition(tempUCDO.getTheMean());
     ...
     EventHandler<MouseEvent> yAxisMouseHandler = new EventHandler<MouseEvent>() {
        ...
        {           
            ...
            if(mouseEvent.getEventType() == MouseEvent.MOUSE_DRAGGED) {
                ...
                switch (choiceOfPlot) {
                    case "BarAndMean": doBarAndMean(); break; 
                    case "StandardDeviation":   // Bar +/- sd
                    case "StandardError":       // Bar +/- se
                    case "MarginOfError":       // Bar +/- me
                        doDotsOrRectsWithErrorBars();
