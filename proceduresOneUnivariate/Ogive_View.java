...
public class Ogive_View extends Region {
    // POJOs
    ...
    public Ogive_View(Ogive_Model ogive_Model, Exploration_Dashboard explore_Dashboard,
        ...
        nLegalDataPoints = ogive_Model.getQDV().getLegalN();
        maxDataValue = ogive_Model.getQDV().getMaxValue();
    ...
    public void completeTheDeal() { 
        ...
        xAxis.setLabel(ogive_Model.getQDV().getDataLabel());
        setUpUI();       
        setUpAnchorPane();
        setHandlers();
        doTheGraph();   
    ...
    public void initializeGraphParameters() {  
        ...
        nCheckBoxes = 3;
        possibleBarsIndex = 3;
        initial_xMin = ogive_Model.getQDV().getMinValue();
        initial_xMax = ogive_Model.getQDV().getMaxValue();
       ...
       // xAxis.setLabel("Points");
        ...
        yAxis.forceHighScaleEndToBe(1.0);
        ...
        yAxis.setLabel(" Cumulative Frequency ");
    }
    ...
    public void doTheGraph() {
        ...
        for (int ithBin = 0; ithBin < nBins; ithBin++ ) {
            ...
            if (binRange[0] <= maxDataValue ) {
                pixSquareLeft = xAxis.getDisplayPosition(binRange[0]);
                pixSquareTop = yAxis.getDisplayPosition(cumulativeFreqs[ithBin]);

                hh = yAxis.getDisplayPosition(0.0) - yAxis.getDisplayPosition(cumulativeFreqs[ithBin]);
    ...
    public void establishBinSizes(int possBarsIndex) { 
        ...
        // --------------------------------------------------
        double tempMin = ogive_Model.getQDV().getMinValue();
        double tempMax = ogive_Model.getQDV().getMaxValue();
        // --------------------------------------------------
        ...
        cumulativeFreqs = new double[nBins + 1];
        ...
        cumulativeFreqs[0] = (double)frequencies[0] / (double)nLegalDataPoints;
        for (int ith_Bin = 1; ith_Bin < nBins; ith_Bin++)
        {
            cumulativeFreqs[ith_Bin] = cumulativeFreqs[ith_Bin - 1] + (double)frequencies[ith_Bin] / (double)nLegalDataPoints;
            if (cumulativeFreqs[ith_Bin] <  cumulativeFreqs[ith_Bin - 1])
            {
                cumulativeFreqs[ith_Bin] = cumulativeFreqs[ith_Bin - 1];
            }
        }
        
        maximumFreq = frequencies[nBins - 1];  // Safety pad
    }
